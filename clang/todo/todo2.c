...
void handle_help() {
    ...
    printf("--list                 - Lists only the task descriptions.\n");
...
void save_tasks(Task tasks[], int count) {
    ...
    if (fp == NULL) {
        perror("Error opening file to write tasks");
        return;
    }
...
void load_tasks(Task tasks[], int *count) {
    ...
    if (fp == NULL) {
        perror("Error opening file to read tasks");
        return;
    }

    while (*count < MAX_TASKS && fscanf(fp, "%d %99s", &tasks[*count].completed, tasks[*count].description) == 2) {
        (*count)++;
    }
...
int main() {
    ...
    while (1) {
        ...
        save_tasks(tasks, count); // Save tasks after each command
    }
