def solve_knapsack(weights, prices, capacity, index, memo):
    ...
    if weights[index] > capacity:
        ...
        memo[(capacity, index)] = solve_knapsack(weights, prices, capacity, index+1, memo)
        return memo[(capacity, index)]
    ...
    memo[(capacity, index)] = max(
        prices[index] +
        solve_knapsack(weights, prices, capacity - weights[index], index+1, memo),
        solve_knapsack(weights, prices, capacity, index + 1, memo)
    )
    return memo[(capacity, index)]
...
def knapsack(weights, prices, capacity):
    ...
    memo = {}
    return solve_knapsack(weights, prices, capacity, 0, memo)
